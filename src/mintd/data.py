...
class CloneResult:
    ...
    ``PullSummary.error_count``), plus every selected sub-path of a
    directory out that was not on disk after the pull. Each was already
    reported via ``reporter.error`` with its own hint; a non-zero count makes
...
class StaleBackupExists(ImportDestinationExists):
    ...
    finished and what is in it may be the last complete copy in the working
    tree. Not the only copy: the DVC cache normally holds the same bytes, which
    is why the message offers `dvc checkout --force` as one way back.
    ...
    "mintd will not touch your last complete working copy", and rendering it with
    ...
    try:
        with status_cm:
            for i, p in enumerate(paths, 1):
                ...
                # left no working copy and said nothing: the bytes were still in
                # the DVC cache, but only a researcher who knew to run `dvc
                # checkout` got them back. `--force` means "overwrite the
                # destination", not "delete it and leave nothing in its place if
                # the network drops". Same helper, same guarantees, as `bump`. The
    ...
    *,
    ...
    if all_outputs:
        ...
        if not selected:
            ...
            #
            # Two different faults reach here and the user cannot act on
            # them the same way, so the message names which one it is: an
            # entry with NO outputs is the producer's to fix, while rows
            # that exist but carry no usable `path` are a malformed entry.
            # And the way out is not `--path` -- `--all` wins over a `--path`
            # the user already passed, so suggesting it reads as advice they
            # just followed. Point at the primary when there is one.
            primary = data_products.get("primary")
            fault = (
                "lists no data_products.outputs"
                if not outputs
                else f"lists {len(outputs)} data_products.outputs, none with a usable path"
            )
            way_out = (
                f"drop --all to import the primary ({primary!r})"
                if isinstance(primary, str) and primary
                else "wait for the producer to publish one"
            )
            raise NoTrackedOutputs(
                f"catalog entry {name!r} {fault}; --all has nothing to "
                f"import ({way_out})"
            )
        return selected
...
def _tracked_output_targets(entry: dict[str, Any]) -> list[str]:
    ...
    data_products = _section(entry, "data_products")
    outputs = _rows(data_products, "outputs")
    # `isinstance(..., str)`, not `"path" in o`: the same row test
    # `_resolve_paths` uses. With the weaker one, `outputs: [{path: 7}]` put a
    # non-string into `tracked`, and `data clone --path x` died in
    # `normalize_target`'s `.replace` with `AttributeError: 'int' object has
    # no attribute 'replace'` instead of listing the real outputs.
    tracked = [
        o["path"]
        for o in outputs
        if isinstance(o, dict) and isinstance(o.get("path"), str)
...
def _resolve_targets_in_clone(
    ...
    for it (`_dvc_state.outs_matching`, three directions) and return
    ``(pull targets, unverified)``: the project-relative targets, sorted and
    de-duplicated, plus every selection that resolved ONLY as a sub-path of
    a directory out — the caller must check those landed (below).
    ...
    `--path` into a whole-directory download. The flip side: a declared
    sub-path the directory out's manifest does NOT contain is accepted by
    dvc too — `dvc pull data/final/stale` fetches the `.dir` manifest, lands
    nothing and exits 0 (the clone-resolver plan's U1). The manifest of an
    md5 `.dir` out is a cache object, not in git, so membership cannot be
    tested before the pull; such selections are returned as ``unverified``
    and `clone_and_pull_product` requires them on disk afterwards.
    ...
    unverified: list[str] = []
    # Re-enumerates the clone once per selector (documented on
    ...
    for declared in targets:
        ...
        if declared_rel not in unverified and all(
            path_covers(out.target, declared_rel) and not path_covers(declared_rel, out.target)
            for out in matched
        ):
            unverified.append(declared_rel)
    return sorted(resolved), unverified
...
def clone_and_pull_product(
    ...
    *,
    ...
    # `--rev` + `--primary`: the primary came from the HEAD catalog while the
    # clone is at an older rev that may simply predate it — both rejection
    # messages below word their cause by this.
    ...
    if targets is not None:
        ...
        try:
            ...
            # before dvc sees it.
            targets, unverified = _resolve_targets_in_clone(
                resolved_dest, targets=targets, name=name, rev_primary=rev_primary,
            )
    ...
    # A selection that resolved only as a sub-path of a directory out was
    ...
    for p in missing:
        if reporter is not None:
            reporter.error(
                f"{p} is declared by the catalog but the producer's directory "
                f"out does not contain it at this rev; nothing was pulled for it",
                hint=(
                    (
                        "the catalog describes HEAD, so this rev may predate "
                        "the primary; "
                        if rev_primary else
                        "the declaration is stale: have the producer re-run "
                        "'mintd check' and 'mintd registry update'; "
                    )
                    + f"the clone is kept — run 'mintd data pull' inside "
                    f"{resolved_dest.name}/ to land the whole directory out instead"
                ),
            )
    ...
    return CloneResult(
        ...
        pull_error_count=pull_summary.error_count + len(missing),
    )
...
def _payload_backup(dest: Path, *, engage: bool, name: str) -> Iterator[None]:
    ...
    transfer that died mid-flight left the payload gone from the working tree
    (recoverable from the DVC cache with `dvc checkout`, but nothing said so)
    and the pointer still at the old rev, from a command the user ran with no
    `--force` anywhere. A
    ...
    # unconditionally one line before the rename, so the retry deleted the last
    # complete working copy and moved the partial corpse into its place (the
    # DVC cache normally still held the bytes; nothing told anyone). Refuse and
    ...
    if engage and (backup.exists() or backup.is_symlink()):
        pointer = dest.with_name(dest.name + ".dvc")
        raise StaleBackupExists(
            f"{backup} already exists: an earlier mintd transfer of {name!r} "
            f"was interrupted and left the previous payload there. Either "
            f"rebuild {dest} from the DVC cache with `dvc checkout --force "
            f"{pointer}` and delete the backup, or replace {dest} with the "
            f"backup; then retry"
        )
