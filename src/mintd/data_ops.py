...
def _pull_isolating_failures(
    ...
    *,
    ...
    """Batch-first, isolate-on-failure ``dvc pull``: one batched argv on the
    happy path; only when it raises is each member retried alone (idempotent —
    dvc skips what the batch already landed), so survivors land and each
    failure is named once. A single-target batch is not retried (the retry
    would be the same argv). Then every target that did not raise is stat-
    ...
    try:
        ...
        if len(targets) == 1:
            failed[targets[0]] = _dvc_failure_why(exc)
        else:
...
def _checkout_pull_verify(
    ...
    *,
    ...
    2. ``dvc pull`` of ``pull_targets`` — non-imports in one batched pull
       whose failures are isolated per target and verified on disk
...
def _pull_targets_with_import_rescue(
    ...
    *,
    ...
    """Pull ``pull_targets``: non-imports in one batched ``dvc pull`` with
    per-target failure isolation, each dvc-import serialized with the rescue
    lane behind a failed/non-materializing pull. Returns the non-imports that
    ...
    # Non-imports: one batched pull, failures isolated per target and
    # verified on disk (never raises for a per-target failure).
