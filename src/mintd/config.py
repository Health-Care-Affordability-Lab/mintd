...
def get_storage_credentials() -> Tuple[str, str]:
    ...
    if KEYRING_AVAILABLE:
        access_key = keyring.get_password("mintd", "aws_access_key_id")
        secret_key = keyring.get_password("mintd", "aws_secret_access_key")

        if access_key and secret_key:
            return access_key, secret_key

    # Fall back to environment variables
    access_key = os.getenv("AWS_ACCESS_KEY_ID") or os.getenv("MINTD_AWS_ACCESS_KEY_ID")
    secret_key = os.getenv("AWS_SECRET_ACCESS_KEY") or os.getenv("MINTD_AWS_SECRET_ACCESS_KEY")

    if access_key and secret_key:
        return access_key, secret_key

    raise ValueError(
        "Storage credentials not found. Please run 'mintd config' to set them up, "
...
def get_registry_url() -> str:
    ...
    url = os.getenv("MINTD_REGISTRY_URL")
    if url:
        return url.strip()
    ...
    raise ValueError(
        "Registry URL not configured. Please set MINTD_REGISTRY_URL environment variable "
        "or configure registry.url in ~/.mintd/config.yaml"
    )
...
def set_storage_credentials(access_key: str, secret_key: str) -> None:
    ...
    keyring.set_password("mintd", "aws_access_key_id", access_key)
    keyring.set_password("mintd", "aws_secret_access_key", secret_key)


def get_config() -> dict:
    """Load configuration from ~/.mintd/config.yaml.
...
def save_config(config: dict) -> None:
    """Save configuration to ~/.mintd/config.yaml.

    Args:
        config: Configuration dictionary to save
    """
    CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    # Ensure secure permissions (rwx------)
...
def init_config() -> None:
    ...
    console.print("\n[bold blue]mintd Configuration Setup[/bold blue]")
    ...
    if gh_path:
        ...
        if not continue_setup:
            console.print("\n[dim]Setup paused. Install GitHub CLI and run 'mintd config setup' again.[/dim]")
    ...
    console.print("\n🎉 Setup complete! You can now use mintd to create projects.")
