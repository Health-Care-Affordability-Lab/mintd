...
def test_a_bump_refuses_a_stale_backup_instead_of_destroying_it(
    ...
    finished, and what is in it may be the last complete working copy (the
    DVC cache normally holds the same bytes).
    ...
    `dest.rename(backup)`, so the retry deleted the last complete working copy and
